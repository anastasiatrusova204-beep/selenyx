...
def _about_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="📝 Написать отзыв", callback_data="cb_feedback")],
    ])
...
@router.callback_query(F.data == "cb_feedback")
async def cb_feedback(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer(
        ...
        "Любое сообщение — отправь его прямо сюда, я всё прочту.",
...
@router.message(UserState.leaving_feedback)
async def handle_feedback(message: Message, state: FSMContext) -> None:
    ...
    feedback_text = (
        ...
        f"Имя: {user.first_name} ({user_info})\n\n"
        f"{message.text}"
    )
    for admin_id in ADMIN_IDS:
        try:
            await message.bot.send_message(admin_id, feedback_text)
        except Exception:
            pass

    await message.answer(
        "Спасибо! Твой отзыв получен 🙏\n\n"
        "Это помогает делать бот лучше.",
        reply_markup=main_menu(),
    )
...
async def handle_resetme(message: Message) -> None:
    """Удалить свои данные из БД (для тестирования первого входа)."""
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("DELETE FROM users WHERE user_id = ?", (message.from_user.id,))
        await db.commit()
    await message.answer("✅ Данные удалены. Отправь /start для первого входа.")
...
async def main() -> None:
    ...
    await bot.set_my_description(
        "🌙 Selenyx\n\n"
        "Бывает: всё делаешь правильно, а день идёт не так.\n"
        "Бывает наоборот — будто ветер в спину.\n\n"
        "Selenyx каждое утро смотрит на реальное положение Луны "
        "и говорит простым языком: что сегодня работает, что лучше отложить.\n\n"
        "Без терминов. Без общих гороскопов.\n"
        "Только твой знак — и конкретный совет на день.\n\n"
        "Жми /start — попробуй сегодня ✨"
    )
    await bot.set_my_short_description(
        "Каждое утро — что сегодня работает и что лучше отложить. По реальному положению Луны."
    )
    ...
    async def _guarded(name: str, coro):
        ...
        try:
            ...
            raise  # нормальное завершение при Ctrl+C
        except Exception as e:
            logger.critical(f"[{name}] завершился с ошибкой: {e}", exc_info=True)

    # return_exceptions=True: падение одной корутины не отменяет вторую
    await asyncio.gather(
        _guarded("API", start_api_server()),
        _guarded("Bot", dp.start_polling(bot, drop_pending_updates=True)),
